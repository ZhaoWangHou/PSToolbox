...
void SCP::GetLargePressureValues(double p_lim, vector<double> &x_crit, vector<double> &p_crit, vector<double> &t_crit,
...
void SCP::GetSmallPressureValues(double p_lim, vector<double> &x_crit, vector<double> &p_crit, vector<double> &t_crit,
                                 vector<string> &ID_crit) {
  double p_min = p_lim;
  int idx = -1;
  for (int i = 0; i < Npts; i++) {
...
string SCP::Info() {
  ...
  if (show_pts) {
    //In case of data needed from the data points, a
    oss << "\n\n # of grid pts.: " << Npts;
...
void SCP::Ini(double dt_target) {
  t = 0.;
  if (fabs(a) < 1.) {
    cout << endl << "WARNING! Sonic velocity in pipe " << name << " is a=" << a << " !!!";
    cin.get();
  }
  ...
  if (Npts < Npts_min) {
    Npts = Npts_min; // a minimum of 20 points is set
    printf(" -> %d", Npts);
  }
  ...
  for (int i = 0; i < Npts; i++) {
    x(i) = i * L / (Npts - 1);
    p(i) = pstart - 0.02 * x(i) / D * ro / 2. * vini * abs(vini);
    v(i) = vini;
  }
  ini_done = true;
...
void SCP::Ini(int Npts_mul) {
  ...
  Npts = Npts_min * Npts_mul; //20 times as many points created
  x = VectorXd::Zero(Npts);
  p = VectorXd::Zero(Npts);
  v = VectorXd::Zero(Npts);
...
double SCP::GetAlphaAtEnd(double t_target) {
  ...
  if (delta_t > dt) {
    if (delta_t - TOL < dt)
      ...
      //if target time is too big, an error occurs
      cout << endl
...
void SCP::GetEdgeEquationCoeffs(double t_target, bool is_front, double &LHS, double &coeff_Q, double &coeff_p1,
                                double &coeff_p2) {
...
float SCP::GetPenult(string what) {
  ...
  int loc = 0;
  if (what == "t") { loc = 0; } else if (what == "p_front") { loc = 1; } else if (what == "p_back") { loc = 2; } else if
  (what == "v_front") { loc = 3; } else if (what == "v_back") { loc = 4; } else if (
    what == "mp_front") { loc = 5; } else if (what == "mp_back") { loc = 6; } else { loc = 0; }
...
void SCP::Save_status(bool newfile, bool atonce) {
  if (!save_data) {
    ...
    if (!newfile) {
      //No new file is created, stuff is added
      tstatus.push_back(t);
      ...
      Tfile = fopen(Tfname.c_str(), "w");
      vfile = fopen(vfname.c_str(), "w");

      fprintf(pfile, "# Npts = %d \n", Npts);
      fprintf(Tfile, "# Npts = %d \n", Npts);
      fprintf(vfile, "# Npts = %d \n", Npts); //Fist line
      for (int i = 0; i < tstatus.size(); i++) {
        fprintf(pfile, "%8.6e", tstatus.at(i));
        fprintf(Tfile, "%8.6e", tstatus.at(i));
        fprintf(vfile, "%8.6e", tstatus.at(i));
    ...
    if (newfile) {
      ...
      fprintf(pfile, "# Npts = %d \n", Npts);
      fprintf(vfile, "# Npts = %d \n", Npts);
      fprintf(pfile, "%8.6e", t);
      fprintf(Tfile, "%8.6e", t);
      fprintf(vfile, "%8.6e", t);
      for (int i = 0; i < Npts; i++) {
        fprintf(pfile, ";%8.6e", p(i));
        fprintf(vfile, ";%8.6e", v(i));
      }
      fprintf(pfile, "%-s \n", "");
      fprintf(vfile, "%-s \n", "");
    } else {
      pfile = fopen(pfname.c_str(), "a");
      vfile = fopen(vfname.c_str(), "a");
      fprintf(pfile, "%8.6e", t);
      fprintf(vfile, "%8.6e", t);
      for (int i = 0; i < Npts; i++) {
        fprintf(pfile, ";%8.6e", p(i));
        fprintf(vfile, ";%8.6e", v(i));
      } //Avoids hanging semicolon at end
      fprintf(pfile, "%-s \n", "");
      fprintf(vfile, "%-s \n", "");
    }
    fclose(pfile);
